...
| `0023_add_companies_parent_account_id.sql` | **NOT YET APPLIED** | `companies.parent_account_id` (self-referencing FK, nullable) + `companies_parent_account_id_not_self` CHECK + index (Layer 12, Multi-Brand Account Grouping). Written and committed to `feat/dashboard-commercial-layers` 2026-07-06; the Supabase MCP `apply_migration` call was explicitly denied by the auto-mode permission classifier ("do everything, all permission approved" does not meet the specificity bar for a live production schema change). App code (`src/lib/db/companies.ts`'s `updateCompanyParent`, the `companies` API route's `parentAccountId` handling, and `CompaniesView`'s grouping UI) is written against this column and will read/write it as `null`/absent gracefully until applied, but the grouping feature is non-functional in production until a human runs this migration (`supabase db push` after review, or paste into the SQL Editor) and updates this row to reflect that. |
| `0024_add_account_plans.sql` | **NOT YET APPLIED** | `account_plans` table (3 seeded tiers: Bronze/Silver/Gold, `job_slot_limit`/`cse_hours_included`/`price_mmk`) + `companies.plan_id` FK (Layer 13, Plan Tiers & Usage Metering). Same situation as 0023 -- written and committed to `feat/dashboard-commercial-layers` 2026-07-06, not applied (same permission-classifier denial). `hasPlanCapacity()` in `src/lib/db/accountPlans.ts` treats a company with no `plan_id` as unmetered, so nothing changes in production until both this migration is applied AND a human deliberately assigns a plan to an account via the Billing tab's new Account Plans panel. `price_mmk` seed values are placeholder zeros -- real tier pricing needs the repo owner's input before this is a real monetization mechanism rather than just job-slot gating. |
