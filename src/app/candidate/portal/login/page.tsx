...
export const metadata: Metadata = {
  title: 'Candidate Portal Sign In | Lion Jobs Agency',
  robots: { index: false },
};

export default async function CandidatePortalLoginPage(
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-brand-50 to-blue-50/40 px-4 dark:from-brand-950/20 dark:to-slate-950">
      <div className="w-full max-w-sm rounded-3xl border border-border bg-card p-8 shadow-xl shadow-brand-600/5">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-foreground">Candidate Portal</h1>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Sign in with the email you used to apply. We&apos;ll send you a secure link — no password needed.
            </p>
          </div>
        </div>
        <PortalLoginForm requestLinkApiPath="/api/candidate-portal/request-link" errorParam={error} />
      </div>
    </div>
  );
}
