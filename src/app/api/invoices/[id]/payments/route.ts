...
import { getInvoiceById, getPaymentsByInvoiceId, recordInvoicePayment, activateFeaturedPlacementIfInvoicePaid, activateJobBoostIfInvoicePaid, activateContactUnlockIfInvoicePaid } from '@/lib/db';
  ...
  try {
    ...
    try {
      ...
      await logFailure({
        category: 'invoicing',
        route:    '/api/invoices/[id]/payments',
        message:  'Payment recorded but contact unlock activation failed',
        error:    activationErr,
        context:  { invoiceId: id, companyId: invoice.companyId },
      });
    }
