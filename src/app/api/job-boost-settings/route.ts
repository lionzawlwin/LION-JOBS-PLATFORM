...
// Same pattern as /api/featured-placement-settings, one price point over --
// 'billing'-scoped slice of the wider agency_settings singleton row.
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    await updateAgencySettings({
      jobBoostPriceMmk:     parsed.data.priceMmk,
      jobBoostDurationDays: parsed.data.durationDays,
    });
    await logAudit({ action: 'update', domain: 'billing', entityType: 'job_boost_settings', entityId: 'default' });
    return Response.json({ ok: true });
  } catch (err) {
    await logFailure({
      category: 'other',
      route:    '/api/job-boost-settings',
      message:  'Could not update job boost settings',
      error:    err,
    });
