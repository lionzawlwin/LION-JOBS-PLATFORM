...
// Mirrors AccountPlansPanel.tsx's PricingEditor -- same inline
// edit/save/cancel pattern, applied to the two Featured Placement fields
// instead of a per-plan price. Per the repo owner's explicit request:
// price and duration were hardcoded constants at launch (with a note that
// the price was a guess needing confirmation); this makes both a data
// write via PATCH /api/featured-placement-settings, never a code change.
export function FeaturedPlacementSettingsPanel() {
  const { settings, loading, mutate } = useFeaturedPlacementSettings();
  ...
  async function save() {
    ...
    setError('');
    try {
      const res = await fetch('/api/featured-placement-settings', {
        method:  'PATCH',
  ...
  return (
    <div className="mb-6 rounded-2xl border border-border bg-card p-5 shadow-sm">
      <h3 className="mb-1 flex items-center gap-2 text-sm font-bold text-foreground">
        <Sparkles size={15} className="text-amber-500" /> Featured Placement Pricing
      </h3>
      <p className="mb-4 text-xs text-muted-foreground">
        What employers see and pay when they request a Featured Placement from their portal. Changing this
        never affects placements already invoiced or active — only new requests approved after the change.
      </p>
