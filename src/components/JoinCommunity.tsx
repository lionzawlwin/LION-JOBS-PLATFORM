// Channel URLs — set these in your .env.local / Vercel environment variables.
// The component still renders with placeholder hrefs if the vars are not set.
const FACEBOOK_URL =
  process.env.NEXT_PUBLIC_FACEBOOK_URL ?? 'https://facebook.com/lionjobsagency';
const VIBER_URL =
  process.env.NEXT_PUBLIC_VIBER_URL ?? 'https://invite.viber.com/?g2=YOUR_VIBER_INVITE';
const TELEGRAM_URL =
  process.env.NEXT_PUBLIC_TELEGRAM_URL ?? 'https://t.me/lionjobschannel';
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {CHANNELS.map((ch) => (
            <a
              ...
              className={`group flex flex-col rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg ring-0 hover:ring-4 ${ch.ring}`}
            >
              {/* Platform icon + name */}
